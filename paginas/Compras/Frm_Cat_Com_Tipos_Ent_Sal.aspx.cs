...
public partial class paginas_Compras_Frm_Cat_Com_Tipos_Ent_Sal : System.Web.UI.Page
{
    #region Variables
    ...
    private void Cargar_Combos()
    {
        ...
        Cmb_Tipos.Items.Add(new ListItem("ENTRADA", "ENTRADA"));
        Cmb_Tipos.Items.Add(new ListItem("SALIDA", "SALIDA"));
    ...
    private void Cargar_Grid(int Page_Index)
    {
        try
        {
            Cls_Cat_Com_Tipos_Entradas_Salidas_Negocio Tipos_Negocio = new Cls_Cat_Com_Tipos_Entradas_Salidas_Negocio();
            Tipos_Negocio.P_Nombre = M_Busqueda;
            Dt_Unidades = Tipos_Negocio.Consulta_Tipo_Movimiento();
            Grid_Tipos.PageIndex = Page_Index;
            Grid_Tipos.DataSource = Dt_Unidades;
            Grid_Tipos.DataBind();
        }
        catch (Exception Ex)
        {
    ...
    private void Cargar_Datos(DataRow Dr_Tipos)
    {
        try
        {
            Txt_Comentarios.Text = Dr_Tipos[Cat_Com_Tipos_Ent_Sal.Campo_Comentarios].ToString();
            Txt_Abreviatura.Text = Dr_Tipos[Cat_Com_Tipos_Ent_Sal.Campo_Abreviatura].ToString();
            Txt_ID.Text = Dr_Tipos[Cat_Com_Tipos_Ent_Sal.Campo_Tipo_Movimiento_ID].ToString();
            Txt_Nombre.Text = Dr_Tipos[Cat_Com_Tipos_Ent_Sal.Campo_Nombre].ToString();
            Cmb_Tipos.SelectedValue = Dr_Tipos[Cat_Com_Tipos_Ent_Sal.Campo_Tipo].ToString();
    ...
    private void Estado_Botones(int P_Estado)
    {
        switch (P_Estado)
        {
            case 0: //Estado inicial
                ...
                Configuracion_Acceso("Frm_Cat_Com_Tipos_Ent_Sal.aspx");
                break;
    ...
    private Boolean Validar_Datos()
    {
        ...
        try
        {
            ...
            {
                Resultado = false;
                Mensaje_Error("Favor de ingresar el nombre del Tipo de Movimiento");
            }
            if (Txt_Abreviatura.Text.Trim() == "")
            {
                Resultado = false;
                Mensaje_Error("Favor de ingresar la Abreviatura del Tipo de Movimiento");
            }
            if (Cmb_Tipos.SelectedIndex <= 0)
            {
                Resultado = false;
                Mensaje_Error("Favor de ingresar el tipo de Movimiento");
            }
    ...
    private void Alta()
    {
        try
        {
            if (Validar_Datos())
            {
                Cls_Cat_Com_Tipos_Entradas_Salidas_Negocio Tipo_Negocio = new Cls_Cat_Com_Tipos_Entradas_Salidas_Negocio();
                if (!Txt_Comentarios.Text.Equals(""))
                {
                    Tipo_Negocio.P_Comentarios = Txt_Comentarios.Text.Trim();
                }
                Tipo_Negocio.P_Abreviatura = Txt_Abreviatura.Text.Trim();
                Tipo_Negocio.P_Tipo = Cmb_Tipos.SelectedValue;
                Tipo_Negocio.P_Nombre = Txt_Nombre.Text.Trim();
                Tipo_Negocio.P_Usuario_Creo = Cls_Sessiones.Nombre_Empleado;
                Tipo_Negocio.Alta_Tipo_Movimiento();
                ScriptManager.RegisterStartupScript(this, this.GetType(), "Catalogo de Tipos de Movimientos", "alert('El Alta del Tipo de Movimiento fue Exitosa');", true);
                Estado_Botones(Const_Estado_Inicial);
                Txt_Busqueda.Text = "";
                Cargar_Grid(0);
            }
    ...
    private void Baja()
    {
        ...
        {
            Cls_Cat_Com_Tipos_Entradas_Salidas_Negocio Tipo_Negocio = new Cls_Cat_Com_Tipos_Entradas_Salidas_Negocio();
            Tipo_Negocio.P_Tipo_Movimiento_ID = HttpUtility.HtmlDecode(Grid_Tipos.SelectedRow.Cells[1].Text.Trim());
            Tipo_Negocio.Baja_Tipo_Movimiento();
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Catalogo de Tipos de Movimientos", "alert('La Baja del Tipo de Movimiento fue Exitosa');", true);
            Estado_Botones(Const_Estado_Inicial);
            Txt_Busqueda.Text = "";
            Cargar_Grid(0);
        }
    ...
    private void Modificar()
    {
        try
        {
            if (Validar_Datos())
            {
                ...
                Tipo_Negocio.P_Tipo = Cmb_Tipos.SelectedValue;
                Tipo_Negocio.P_Usuario_Creo = Cls_Sessiones.Nombre_Empleado;
                Tipo_Negocio.Cambio_Tipo_Movimiento();
                ScriptManager.RegisterStartupScript(this, this.GetType(), "Catalogo de Unidades", "alert('La modificación del tipo de Movimiento Exitosa');", true);
                Estado_Botones(Const_Estado_Inicial);
                Txt_Busqueda.Text = "";
                Cargar_Grid(0);
            }
    ...
    protected void Btn_Modificar_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            ...
            {
                Mensaje_Error("Favor de seleccionar el Tipo de Movimiento a modificar");
            }
    ...
    protected void Btn_Eliminar_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            ...
            {
                Mensaje_Error("Favor de seleccionar el Tipo de Movimiento a eliminar");
            }
        }
        catch (Exception Ex)
        {
    ...
    #region Eventos Grid
    protected void Grid_Tipos_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        try
        {
            Grid_Tipos.SelectedIndex = (-1);
    ...
    protected void Grid_Tipos_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {            
            if (Grid_Tipos.SelectedIndex > (-1))
            {
                Cargar_Datos(Dt_Unidades.Rows[Grid_Tipos.SelectedIndex + (Grid_Tipos.PageIndex * 5)]);
            }
        }
        catch (Exception Ex)
        {
