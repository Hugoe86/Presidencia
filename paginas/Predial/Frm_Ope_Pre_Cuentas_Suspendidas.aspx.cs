...
public partial class paginas_Predial_Frm_Ope_Pre_Cuentas_Suspendidas : System.Web.UI.Page
{
    #region Variables
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                Session["ESTATUS_CUENTAS"] = "IN ('SUSPENDIDA','ACTIVA','VIGENTE')";
    ...
    private void Limpiar_Todo()
    {
        ...
        //Aplica
        Chk_Aplica_Traslado.Checked = false;
        Chk_Aplica_Predial.Checked = false;
    }
    ///****************************************************************************************
    ...
    private void Estado_Botones(int P_Estado)
    {
        ...
        switch (P_Estado)
        {
            case 0: //Estado inicial                
                ...
                Chk_Aplica_Traslado.Enabled = false;
                Chk_Aplica_Predial.Enabled = false;
            ...
            case 3: //Mod
                ...
                Estado = false;
                Chk_Aplica_Traslado.Enabled = true;
                Chk_Aplica_Predial.Enabled = true;
                Cmb_Estatus.Enabled = true;
                Cmb_Estatus.Font.Bold = true;
                //Cmb_Estatus.Text = "VIGENTE";
                Txt_Cuenta_Predial.Enabled = false;
                Btn_Mostrar_Busqueda_Cuentas.Enabled = false;
                Btn_Salir.Enabled = true;
    ...
    protected void Btn_Mostrar_Busqueda_Cuentas_Click(object sender, ImageClickEventArgs e)
    {
        ...
        try
        {
            ...
            if (Session["CUENTA_PREDIAL_ID"] != null && Session["CUENTA_PREDIAL_ID"].ToString().Trim().Length > 0)
            {
                ...
                Chk_Aplica_Traslado.Enabled = true;
                Chk_Aplica_Predial.Enabled = true;
                Cmb_Estatus.SelectedValue = "SUSPENDIDA";
                Cmb_Estatus.Enabled = true;
        ...
        try
        {
            ...
            else
            {
                if (!String.IsNullOrEmpty(Hdn_Cuenta_ID.Value))
                    if (Chk_Aplica_Traslado.Checked || Chk_Aplica_Predial.Checked)
                        Alta_Bloqueo(0);
                    else
                        throw new Exception("Debe Seleccionar una Opción como minimo para aplicar la Suspención.");
            }
        }
        catch (Exception Ex)
        {
    ...
    {
        Txt_Cuenta_Predial.Text = Grid.Rows[Grid.SelectedIndex + (Grid.PageIndex * Grid.PageSize)].Cells[1].Text;
        Hdn_Cuenta_ID.Value = Grid.SelectedDataKey["ID"].ToString();
        Session["CUENTA_PREDIAL_ID"] = Hdn_Cuenta_ID.Value.ToString();
    ...
    ///******************************************************************************* 
    ///NOMBRE DE LA FUNCIÓN : Cargar_Grid_Cuentas_canceladas
    ///DESCRIPCIÓN          : 
    ///PARAMETROS: 
    ///CREO                 : Jesus Toledo
    ...
    private void Cargar_Grid_Cuentas_canceladas(int Page_Index)
    {
        ...
        try
        {
            ...
            Consulta_Cuentas_Canceladas.P_Filtros_Dinamicos = " CUEN." + Cat_Pre_Cuentas_Predial.Campo_Estatus + " = 'SUSPENDIDA' ";
    ...
    private void Alta_Bloqueo(int modo)
    {
        try
        {
            ...
            {
                Cuenta.P_Estatus = Cmb_Estatus.SelectedValue.ToString();
                if (Chk_Aplica_Traslado.Checked && Chk_Aplica_Predial.Checked)
                {
                    Cuenta.P_Tipo_Suspencion = "AMBAS";
                }
                else if (Chk_Aplica_Traslado.Checked)
                {
                    Cuenta.P_Tipo_Suspencion = "TRASLADO";
                }
                else if (Chk_Aplica_Predial.Checked)
                {
                    Cuenta.P_Tipo_Suspencion = "PREDIAL";
                }
                else
                {
                    Cuenta.P_Tipo_Suspencion = "";
                    Cuenta.P_Estatus = "VIGENTE";
                }
                Cuenta.P_Usuario = Cls_Sessiones.Nombre_Empleado;
                Cuenta.Modifcar_Cuenta();
                ScriptManager.RegisterStartupScript(this, this.GetType(), "Catalogo de Conceptos", "alert('La cuenta se Modificó Exitosamente');", true);
                Limpiar_Todo();
                Estado_Botones(Const_Estado_Inicial);
                Cargar_Grid_Cuentas_canceladas(0);
            }
            else
                Mensaje_Error("Seleccione el estatus de la cuenta");
        }
        catch (Exception Ex)
        {
    ...
    private void Cargar_Generales_Cuenta(DataTable dataTable)
    {
        try
        {
            ...
            {
                Cmb_Estatus.SelectedValue = dataTable.Rows[0][Cat_Pre_Cuentas_Predial.Campo_Estatus].ToString().Trim();
            ...
            if (dataTable.Rows[0][Cat_Pre_Cuentas_Predial.Campo_Tipo_Suspencion] != null && dataTable.Rows[0][Cat_Pre_Cuentas_Predial.Campo_Tipo_Suspencion].ToString().Trim().Length > 0)
            {
                if (dataTable.Rows[0][Cat_Pre_Cuentas_Predial.Campo_Tipo_Suspencion].ToString() == "AMBAS" || dataTable.Rows[0][Cat_Pre_Cuentas_Predial.Campo_Tipo_Suspencion].ToString() == "TRASLADO")
                {
                    Chk_Aplica_Traslado.Checked = true;
                }
                if (dataTable.Rows[0][Cat_Pre_Cuentas_Predial.Campo_Tipo_Suspencion].ToString() == "AMBAS" || dataTable.Rows[0][Cat_Pre_Cuentas_Predial.Campo_Tipo_Suspencion].ToString() == "PREDIAL")
                {
                    Chk_Aplica_Predial.Checked = true;
                }
            }
        }
        catch (Exception Ex)
