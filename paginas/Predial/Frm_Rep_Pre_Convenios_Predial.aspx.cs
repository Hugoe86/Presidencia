...
public partial class paginas_Predial_Frm_Rep_Pre_Convenios_Predial : System.Web.UI.Page
{
...
#region METODOS
    ...
    private DataSet Crear_Ds_Exportar_Reporte()
    {
        ...
        if (Cmb_Incluir_Reestructuras.SelectedIndex > 0)
        {
            ...
            if (Cmb_Incluir_Reestructuras.SelectedIndex == 1)
            {
                ...
                Mensaje_Incluidos = "Convenios de predial" + Mensaje_Incluidos;
            }
            ...
            else if (Cmb_Incluir_Reestructuras.SelectedIndex == 2)
            {
                ...
                Mensaje_Incluidos = "Reestructuras de predial" + Mensaje_Incluidos;
            }
        }
        else
        {
            Mensaje_Incluidos = "Convenios y reestructuras de predial" + Mensaje_Incluidos;
        }

        // agregar datos generales al dataset
        var Dr_Convenio = Ds_Convenio.Tables["Dt_Generales"].NewRow();
        Dr_Convenio["Titulo"] = "Convenios de predial";
        ...
        Ds_Convenio.Tables["Dt_Generales"].Rows.Add(Dr_Convenio);

        // llamar metodo de consulta
        Consulta_Convenios.P_Ordenar_Dinamico = Cmb_Ordenar_Campo.SelectedValue + " " + Cmb_Orden_Asc_Desc.SelectedValue;
        Dt_Convenios = Consulta_Convenios.Consultar_Convenios_Predial_Reporte().Copy();
    ...
    ///******************************************************************************* 
    ///NOMBRE DE LA FUNCIÓN : Crear_Ds_Exportar_Reporte_Detallado
    ///DESCRIPCIÓN          : Crea un Dataset con los datos de la consulta de convenios
    ///PARAMETROS: 
    ///CREO                 : Roberto Gonzalez Oseguera
    ///FECHA_CREO           : 11-ene-2012
    ...
    ///CAUSA_MODIFICACIÓN:
    ///*******************************************************************************
    private DataSet Crear_Ds_Exportar_Reporte_Detallado()
    {
        var Consulta_Convenios = new Cls_Rep_Pre_Convenios_Negocio();
        var Ds_Convenio = new Ds_Rpt_Rep_Convenio_Predial_Detallado();
        DataTable Dt_Convenios;
        DateTime Desde_Fecha;
        DateTime Hasta_Fecha;
        String Mensaje_Titulo = "";
        ...
        if (Cmb_Incluir_Reestructuras.SelectedIndex > 0)
        {
            ...
            if (Cmb_Incluir_Reestructuras.SelectedIndex == 1)
            {
                //  pasar el dato a la capa de negocio para la consulta
                Consulta_Convenios.P_Solo_Convenios = true;
                Mensaje_Incluidos = "Convenios de predial Detallados " + Mensaje_Incluidos;
            }
            ...
            else if (Cmb_Incluir_Reestructuras.SelectedIndex == 2)
            {
                //  pasar el dato a la capa de negocio para la consulta
                Consulta_Convenios.P_Solo_Reestructuras = true;
                Mensaje_Incluidos = "Reestructuras de predial Detallados " + Mensaje_Incluidos;
            }
        }
        else
        {
        ...
        // llamar metodo de consulta
        Dt_Convenios = Consulta_Convenios.Consultar_Convenios_Predial_Detallado_Reporte().Copy();
        Dt_Convenios.TableName = "Dt_Convenios_Predial";
        // agregar tabla obtenida de la consulta al dataset
        if (Dt_Convenios != null)
        {
...
#region EVENTOS
    ...
    protected void Btn_Exportar_pdf_Click(object sender, ImageClickEventArgs e)
    {
        ...
        try
        {
            if (Cmb_Tipo.SelectedIndex == 0)
                Exportar_Reporte(Crear_Ds_Exportar_Reporte(), "Rpt_Rep_Pre_Convenios_Predial.rpt", "Convenios_Predial", "pdf", ExportFormatType.PortableDocFormat);
            else
                Exportar_Reporte(Crear_Ds_Exportar_Reporte_Detallado(), "Rpt_Rep_Pre_Convenios_Predial_Detallado.rpt", "Convenios_Predial", "pdf", ExportFormatType.PortableDocFormat);
        }
        catch (Exception Ex)
        {
            Lbl_Mensaje_Error.Visible = true;
    ...
    protected void Btn_Exportar_Excel_Click(object sender, ImageClickEventArgs e)
    {
        ...
        {
            if (Cmb_Tipo.SelectedIndex == 0)
                Exportar_Reporte(Crear_Ds_Exportar_Reporte(), "Rpt_Rep_Pre_Convenios_Predial.rpt", "Convenios_Predial", "xls", ExportFormatType.Excel);
            else
                Exportar_Reporte(Crear_Ds_Exportar_Reporte_Detallado(), "Rpt_Rep_Pre_Convenios_Predial_Detallado.rpt", "Convenios_Predial", "xls", ExportFormatType.Excel);
        }
        catch (Exception Ex)
        {
            Lbl_Mensaje_Error.Visible = true;
