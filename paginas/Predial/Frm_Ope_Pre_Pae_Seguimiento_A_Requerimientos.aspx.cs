...
public partial class paginas_Predial_Frm_Ope_Pre_Pae_Seguimiento_A_Requerimientos : System.Web.UI.Page
{
    #region Page_Load
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            Ventana_Modal = "Abrir_Ventana_Modal('Ventanas_Emergentes/Frm_Busqueda_Contribuyentes_PAE.aspx', 'center:yes;resizable:yes;status:no;dialogWidth:580px;dialogHeight:450px;dialogHide:true;help:no;scroll:no');";
    ...
    private void Limpiar_Formulario()
    {
        ...
        Grid_Requerimientos_Generados.DataSource = null;
    }
    ///****************************************************************************************
    ...
    private bool Validar_Componentes()
    {
        ...
        if (Grid_Requerimientos_Generados.Rows.Count < 1)
        {
    ...
    ///CAUSA_MODIFICACIÓN:
    ///*******************************************************************************
    protected DataTable Crear_Tabla_Requerimientos_Generados()
    {
    ...
    ///CAUSA_MODIFICACIÓN:
    ///*******************************************************************************
    protected void Llenar_DataRow_Requerimientos(DataTable Dt_Generadas, DataTable Dt_Busqueda, Int32 Cont_Busqueda)
    {
    ...
    ///******************************************************************************* 
    ///NOMBRE DE LA FUNCIÓN: Buscar_Requerimientos
    ...
    ///CAUSA_MODIFICACIÓN:
    ///*******************************************************************************
    public void Buscar_Requerimientos()
    {
        Cls_Ope_Pre_Pae_Etapas_Negocio Etapas_PAE = new Cls_Ope_Pre_Pae_Etapas_Negocio();
        DataTable Dt_Cuentas_Predial;
        DataTable Dt_Requerimientos_Generados = Crear_Tabla_Requerimientos_Generados();//Se crea la tabla para pasarla algrid
            ...
            if (Comprobacion_Termida != true)
            {
                if (Session["CUENTAS_PREDIAL_CONTRIBUYENTE"] != null && Session["CUENTAS_PREDIAL_CONTRIBUYENTE"].ToString() != "")
                {
                    ...
                    for (int Cont_Contribuyente = 0; Cont_Contribuyente < Dt_Cuentas_Predial.Rows.Count; Cont_Contribuyente++)
                    {
                        ...
                        {
                            Llenar_DataRow_Requerimientos(Dt_Requerimientos_Generados, Dt_Busqueda, Contador);
                        }
                    }
                }
                else
                ...
                //Comprobar fechas
                for (int Cont_Fechas = 0; Cont_Fechas < Dt_Requerimientos_Generados.Rows.Count; Cont_Fechas++)
                {
                    Comprobar_Filtros_Busqueda(Dt_Requerimientos_Generados, Cont_Borrado, Cont_Fechas);
                }

                Grid_Requerimientos_Generados.DataSource = Dt_Requerimientos_Generados;
                Grid_Requerimientos_Generados.DataBind();
                Session["Grid_Requerimientos_Generados"] = Dt_Requerimientos_Generados;
                Session.Remove("CUENTAS_PREDIAL_CONTRIBUYENTE");
                if (Dt_Requerimientos_Generados.Rows.Count < 1)
                {
                    Mensaje_Error("No se encontraron Requeremientos con esos parametros");
                }
            }
        }
        catch (Exception ex)
    ...
    ///FECHA_CREO: 09/03/2012 04:14:00 p.m.
    ///MODIFICO: 
    ///FECHA_MODIFICO:
    ///CAUSA_MODIFICACIÓN:
    ///*******************************************************************************
    ...
    ///CAUSA_MODIFICACIÓN:
    ///*******************************************************************************
    protected void Btn_Busca_Requerimiento_Click(object sender, ImageClickEventArgs e)
    {
    ...
    #region Grid
    ///******************************************************************************* 
    ///NOMBRE DE LA FUNCIÓN : Grid_Requerimientos_Generados_SelectedIndexChanging
    ...
    ///CAUSA_MODIFICACIÓN:
    ///*******************************************************************************
    protected void Grid_Requerimientos_Generados_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        try
        {
            Grid_Requerimientos_Generados.PageIndex = e.NewPageIndex;
            Grid_Requerimientos_Generados.DataSource = Session["Grid_Requerimientos_Generados"];
            Grid_Requerimientos_Generados.DataBind();
        }
        catch (Exception ex) { Mensaje_Error(ex.Message); }
    }
    ///******************************************************************************* 
    ///NOMBRE DE LA FUNCIÓN : Grid_Requerimientos_Generados_SelectedIndexChanged
    ...
    ///CAUSA_MODIFICACIÓN:
    ///*******************************************************************************
    protected void Grid_Requerimientos_Generados_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        DataTable Dt_Requerimientos_Generados = (DataTable)Session["Grid_Requerimientos_Generados"];
        ...
        if (Dt_Honorarios != null || Dt_Publicaciones != null || Dt_Notificaciones != null)
        {
            try
            {
                No_Detalle_Etapa = Dt_Requerimientos_Generados.Rows[Grid_Requerimientos_Generados.SelectedIndex + (Grid_Requerimientos_Generados.PageSize * Grid_Requerimientos_Generados.PageIndex)]["NO_DETALLE_ETAPA"].ToString();
                if (Dt_Notificaciones != null)
                {
                    ...
                    Rs_Notificaciones.P_Proceso = "REQUERIMIENTO";
                    Estatus = Dt_Notificaciones.Rows[0]["ESTATUS"].ToString();
                    Rs_Notificaciones.Alta_Notificaciones();
                }
                if (Estatus == "")
                {
                    Estatus = Dt_Requerimientos_Generados.Rows[Grid_Requerimientos_Generados.SelectedIndex + (Grid_Requerimientos_Generados.PageSize * Grid_Requerimientos_Generados.PageIndex)]["ESTATUS"].ToString();
                }
                if (Dt_Honorarios != null)
                {
                    for (int Conta_Ejecucion = 0; Conta_Ejecucion < Dt_Honorarios.Rows.Count; Conta_Ejecucion++)
                    {
                        ...
                        Rs_Honorarios.P_Proceso = "REQUERIMIENTO";
                ...
                if (Dt_Publicaciones != null)
                {
                    for (int Conta_Publicacion = 0; Conta_Publicacion < Dt_Publicaciones.Rows.Count; Conta_Publicacion++)
                    {
                        ...
                        Rs_Publicaciones.P_Proceso = "REQUERIMIENTO";
                ...
                int Pagina = Grid_Requerimientos_Generados.PageIndex;
                Buscar_Requerimientos();
                Session["Generadas"] = Dt_Requerimientos_Generados;
                Grid_Requerimientos_Generados.PageIndex = Pagina;
            }
            catch (Exception Ex)
            {
