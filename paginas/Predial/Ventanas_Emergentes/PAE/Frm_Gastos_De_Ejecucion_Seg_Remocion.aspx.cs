...
public partial class paginas_Predial_Ventanas_Emergentes_PAE_Frm_Gastos_De_Ejecucion_Seg_Remocion : System.Web.UI.Page
{
    ...
    //////******************************************************************************* 
    ///NOMBRE DE LA FUNCIÓN: Cargar_Combo_Depositario
    ///DESCRIPCIÓN: Metodo usado para cargar el listado de los tipos de depositarios
    ///PARAMETROS: 
    ///CREO: Angel Antonio Escamilla Trejo
    ///FECHA_CREO: 23/03/2012 05:30:12 p.m.
    ///MODIFICO: 
    ///FECHA_MODIFICO:
    ///CAUSA_MODIFICACIÓN:
    ///*******************************************************************************
    private void Cargar_Combo_Depositario()
    {
        try
        {
            Cmb_Depositario.Items.Add(new ListItem("<-- SELECCIONE -->", "0"));
            Cmb_Depositario.Items.Add(new ListItem("DEPOSITARIO", "1"));
        }
        catch (Exception ex)
        {
            Mensaje_Error(ex.Message.ToString());
    ...
    ///******************************************************************************* 
    ///NOMBRE DE LA FUNCIÓN : Crear_Tabla_Depositario
    ///DESCRIPCIÓN          : Devuelve un DataTable con la estructura para el cambio de depositario
    ///PARAMETROS: 
    ///CREO                 : Angel E. Trejo
    ///FECHA_CREO           : 16/03/2012 12:23:00 p.m.
    ///MODIFICO: 
    ///FECHA_MODIFICO:
    ///CAUSA_MODIFICACIÓN:
    ///*******************************************************************************
    protected DataTable Crear_Tabla_Depositario()
    {
        DataTable Dt_Depositario = new DataTable();
        Dt_Depositario.Columns.Add(new DataColumn("DEPOSITARIO", typeof(String)));
        Dt_Depositario.Columns.Add(new DataColumn("NOMBRE", typeof(String)));
        Dt_Depositario.Columns.Add(new DataColumn("DOMICILIO", typeof(String)));
        return Dt_Depositario;

    }
    ///******************************************************************************* 
    ///NOMBRE DE LA FUNCIÓN : Llenar_DataRow_Depositario
    ///DESCRIPCIÓN          : Agrega una nueva fila para el cambio de depositario
    ///PARAMETROS: 
    ///CREO                 : Angel E. Trejo
    ///FECHA_CREO           : 16/03/2012 12:28:00 p.m.
    ///MODIFICO: 
    ///FECHA_MODIFICO:
    ///CAUSA_MODIFICACIÓN:
    ///*******************************************************************************
    protected void Llenar_DataRow_Depositario(DataTable Dt_Depositario)
    {
        DataRow Dr_Depositario;
        Dr_Depositario = Dt_Depositario.NewRow();
        Dr_Depositario["DEPOSITARIO"] = Cmb_Depositario.SelectedItem.Text;
        Dr_Depositario["NOMBRE"] = Txt_Nombre.Text;
        Dr_Depositario["DOMICILIO"] = Txt_Domicilio.Text;
        Dt_Depositario.Rows.Add(Dr_Depositario);
    }
    #endregion
    #region Eventos
    ///*******************************************************************************
    ...
    protected void Btn_Aceptar_Click(object sender, ImageClickEventArgs e)
    {
        ...
        if (Validar_y_Guardar_Estatus())
        {
            ...
            if (Dt_Depositario != null && Dt_Depositario.Rows.Count < 1) { Dt_Depositario = null; }

            if (Dt_Honorarios != null || Dt_Publicaciones != null || Dt_Notificaciones != null || Dt_Depositario != null)
            {
    ...
    ///******************************************************************************* 
    ///NOMBRE DE LA FUNCIÓN : Btn_Agregar_Depositario_Click
    ///DESCRIPCIÓN          : Llena el Grid de los Depositarios
    ///PARAMETROS: 
    ///CREO                 : Angel Antonio Escamilla Trejo
    ///FECHA_CREO           : 16/03/2012 12:37:00 p.m.
    ///MODIFICO: 
    ///FECHA_MODIFICO:
    ///CAUSA_MODIFICACIÓN:
    ///*******************************************************************************
    protected void Btn_Agregar_Depositario_Click(object sender, ImageClickEventArgs e)
    {
        DataTable Dt_Depositario;
        if (Session["Depositarios"] != null)
        {
            Dt_Depositario = (DataTable)Session["Depositarios"];
        }
        else
        {
            Dt_Depositario = Crear_Tabla_Depositario();
        }
        if (Txt_Nombre.Text != "" && Txt_Domicilio.Text != "" && Cmb_Depositario.SelectedIndex > 0)
        {
            Llenar_DataRow_Depositario(Dt_Depositario);
            Grid_Cambio_Depositario.DataSource = Dt_Depositario;
            Grid_Cambio_Depositario.DataBind();
            Session["Depositarios"] = Dt_Depositario;
        }
        Cmb_Depositario.SelectedIndex = 0;
        Txt_Nombre.Text = "";
        Txt_Domicilio.Text = "";
    }
    #endregion
    ...
    ///******************************************************************************* 
    ///NOMBRE DE LA FUNCIÓN : Grid_Cambio_Depositario_PageIndexChanging
    ///DESCRIPCIÓN          : Cambia de pagina en el Grid Depositarios
    ///PARAMETROS: 
    ///CREO                 : Armando Zavala Moreno
    ///FECHA_CREO           : 08/03/2012 05:53:00 p.m.
    ///MODIFICO: 
    ...
    ///CAUSA_MODIFICACIÓN:
    ///*******************************************************************************
    protected void Grid_Cambio_Depositario_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        try
        {
            Grid_Cambio_Depositario.PageIndex = e.NewPageIndex;
            Grid_Cambio_Depositario.DataSource = Session["Depositarios"];
            Grid_Cambio_Depositario.DataBind();
        }
        catch (Exception Ex)
        {
    ...
    ///CAUSA_MODIFICACIÓN:
    ///*******************************************************************************
    protected void Grid_Cambio_Depositario_SelectedIndexChanged(object sender, EventArgs e)
    {
        DataTable Dt_Depositario = (DataTable)Session["Depositarios"];
        Dt_Depositario.Rows.RemoveAt(Grid_Cambio_Depositario.SelectedIndex);
        Grid_Cambio_Depositario.DataSource = Dt_Depositario;
        Grid_Cambio_Depositario.DataBind();
        Session["Depositarios"] = Dt_Depositario;
    }
