...
public partial class paginas_Catastro_Frm_Ope_Cat_Autorizacion_Avaluo_Urbano : System.Web.UI.Page
{
    ...
    private void Llenar_Combo_Firmante()
    {
        ...
        try
        {
            ...
            DataRow Dr_Renglon_Nuevo;
            Cls_Cat_Cat_Motivos_Avaluo_Negocio Firmante = new Cls_Cat_Cat_Motivos_Avaluo_Negocio();
                ...
                if (Grid_Avaluos_Urbanos.SelectedRow.Cells[8].Text == "POR AUTORIZAR")
                {
                    ...
                    else
                    {
                        ...
                        if ((Avaluo.Modificar_Observaciones_Avaluo_Au()))
                        {
                            ...
                            {
                                ScriptManager.RegisterStartupScript(this, this.GetType(), "Avalúo Urbano", "alert('El Avalúo se ha Autorizado Exitosamente');", true);
                                //Insertar_Pasivo("AU" + Txt_No_Avaluo.Text);
                                Imprimir_Reporte_Enviar(Crear_Ds_Avaluo_Urbano(), "Rpt_Ope_Cat_Folio_Pago_Avaluo.rpt", "Rpt_Folio_Pago_Avaluo_Urbano", "Window_Frm", "Avaluo");
                            }
    ...
    ///******************************************************************************* 
    ///NOMBRE DE LA FUNCIÓN : Imprimir_Reporte_Enviar
    ///DESCRIPCIÓN          : Genera el reporte de Crystal con los datos proporcionados en el DataTable y lo manda a la impresora default
    ///PARAMETROS: 
    ///CREO                 : Antonio Salvador Benavides Guardado
    ///FECHA_CREO           : 29/Julio/2011
    ...
    ///CAUSA_MODIFICACIÓN:
    ///*******************************************************************************
    private void Imprimir_Reporte_Enviar(DataSet Ds_Convenios, String Nombre_Reporte, String Nombre_Archivo, String Formato, String Tipo)
    {
        ReportDocument Reporte = new ReportDocument();
        String File_Path = Server.MapPath("../Rpt/Catastro/" + Nombre_Reporte);
        try
    ...
    private void Enviar_Correo_Cuenta_Rechazo()
    {
        ...
        Contenido = "Su avalúo urbano con folio: " + Txt_No_Avaluo.Text + " y clave de trámite: " + Folio + " ha sido Rechazado. Favor de corregirlo.";
        try
        {
            String Perito_Id = Obtener_Dato_Consulta("SELECT " + Ope_Tra_Solicitud.Campo_Inspector_ID + " FROM " + Ope_Tra_Solicitud.Tabla_Ope_Tra_Solicitud + " WHERE " + Ope_Tra_Solicitud.Campo_Solicitud_ID + " = '" + Hdf_Solicitud_Id.Value + "'");
            String E_Mail = Obtener_Dato_Consulta("SELECT " + Cat_Cat_Peritos_Externos.Campo_Usuario + " FROM " + Cat_Cat_Peritos_Externos.Tabla_Cat_Cat_Peritos_Externos + " WHERE " + Cat_Cat_Peritos_Externos.Campo_Perito_Externo_Id + " = '" + Perito_Id + "'").ToUpper();
            if (E_Mail.Trim().Length > 0)
            {
                ...
                mail.P_Password = Cls_Cat_Ate_Peticiones_Datos.Consulta_Parametros().Rows[0][Apl_Parametros.Campo_Password_Correo].ToString();
                mail.P_Recibe = E_Mail.Trim();
                mail.P_Subject = "Avalúo urbano Rechazado " + Txt_No_Avaluo.Text + " - " + Folio;
                mail.P_Texto = Contenido;
